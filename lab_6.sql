...
create procedure usp_top1DUAN
as
begin
	declare @tennv nvarchar(30), @manv char(9)
	select top 1 @manv = nv.MANV from NHANVIEN as nv
				inner join PHANCONG as pc on pc.MA_NVIEN = nv.MANV
				inner join DEAN as da on da.MADA = pc.MADA
				where year(da.NGAYBD) = 2013
				group by nv.MANV
				order by count(da.MADA) desc
	set @tennv = (select ten from NHANVIEN where MANV = @manv)
	print @tennv + N' tham gia nhiều dự án nhất'
end
exec usp_top1DUAN

-- b. Hiển thị tên dự án, trưởng dự án và số nhân viên tham gia dự án
CREATE PROCEDURE usp_ProjectSummary
AS
BEGIN
    SELECT
        d.TENDA AS TenDuAn,
        leader.HONV + ' ' + leader.TENLOT + ' ' + leader.TEN AS TruongDuAn,
        COUNT(pc.MA_NVIEN) AS SoNhanVien
    FROM DEAN d
    LEFT JOIN PHANCONG pc ON d.MADA = pc.MADA
    LEFT JOIN NHANVIEN nv ON pc.MA_NVIEN = nv.MANV
    LEFT JOIN (
        SELECT MADA, MA_NVIEN
        FROM PHANCONG
        WHERE VITRI = N'Trưởng Dự Án'
    ) ld ON d.MADA = ld.MADA
    LEFT JOIN NHANVIEN leader ON ld.MA_NVIEN = leader.MANV
    GROUP BY d.TENDA, leader.HONV, leader.TENLOT, leader.TEN;
END
GO

-- c. Hiển thị tất cả nhân viên tham gia dự án theo mã dự án
CREATE PROCEDURE usp_ListEmployeesByProject
    @MADA INT
AS
BEGIN
    SELECT
        nv.HONV + ' ' + nv.TENLOT + ' ' + nv.TEN AS HoTen,
        pc.VITRI
    FROM PHANCONG pc
...
-- d. Hiển thị tên phòng, số lượng nhân viên và số địa điểm theo mã phòng
CREATE PROCEDURE usp_DepartmentStats
    @MAPB INT
AS
BEGIN
    DECLARE @TenPB NVARCHAR(30);
    SELECT @TenPB = TENPHONG FROM PHONGBAN WHERE MAPHG = @MAPB;
    IF @TenPB IS NULL
    BEGIN
    ...
    SELECT
        @TenPB AS TenPhong,
        (SELECT COUNT(*) FROM NHANVIEN WHERE PHG = @MAPB) AS SoNhanVien,
        (SELECT COUNT(*) FROM DIADIEM_PHG WHERE MAPHG = @MAPB) AS SoDiaDiem;
END
GO

-- e. Hiển thị tên dự án mà nhân viên tham gia ở vị trí truyền vào
CREATE PROCEDURE usp_ProjectsByEmployeeAndRole
    @MANV CHAR(9),
    @VITRI NVARCHAR(50)
AS
BEGIN
    SELECT d.TENDA
    FROM PHANCONG pc
    INNER JOIN DEAN d ON pc.MADA = d.MADA
    WHERE pc.MA_NVIEN = @MANV
      AND pc.VITRI = @VITRI;
END
GO

-- f. Lấy lương cao nhất và tên nhân viên trong phòng
CREATE PROCEDURE usp_MaxSalaryInDept
    @MAPB INT,
    @LuongCaoNhat FLOAT OUTPUT,
    @TenNhanVien NVARCHAR(100) OUTPUT
AS
BEGIN
    SELECT TOP 1
        @LuongCaoNhat = nv.LUONG,
        @TenNhanVien = nv.HONV + ' ' + nv.TENLOT + ' ' + nv.TEN
    FROM NHANVIEN nv
    WHERE nv.PHG = @MAPB
...
-- g. Đếm số dự án bắt đầu và kết thúc trong khoảng thời gian
CREATE PROCEDURE usp_CountProjectsByDateRange
    @NgayBatDau DATETIME,
...
BEGIN
    SELECT @SoDuAn = COUNT(*)
    FROM DEAN
    WHERE NGAYBD >= @NgayBatDau
      AND NGAYKT <= @NgayKetThuc;
END
GO

-- h. Đếm số nhân viên nam và nữ tham gia dự án
CREATE PROCEDURE usp_GenderCountByProject
    @MADA INT,
    @SoNu INT OUTPUT,
    @SoNam INT OUTPUT
AS
BEGIN
    SELECT
        @SoNu = SUM(CASE WHEN nv.PHAI = N'Nữ' THEN 1 ELSE 0 END),
        @SoNam = SUM(CASE WHEN nv.PHAI = N'Nam' THEN 1 ELSE 0 END)
    FROM PHANCONG pc
    INNER JOIN NHANVIEN nv ON pc.MA_NVIEN = nv.MANV
    WHERE pc.MADA = @MADA;
END
...
-- i. Thêm mới phòng ban với kiểm tra
...
AS
BEGIN
    IF EXISTS(SELECT 1 FROM PHONGBAN WHERE MAPHG = @MAPB)
    BEGIN PRINT N'Mã phòng đã tồn tại.'; RETURN; END;
    IF EXISTS(SELECT 1 FROM PHONGBAN WHERE TENPHONG = @TenPB)
    BEGIN PRINT N'Tên phòng đã tồn tại.'; RETURN; END;
    IF NOT EXISTS(SELECT 1 FROM NHANVIEN WHERE MANV = @TrPHG)
    BEGIN PRINT N'Mã trưởng phòng không phải nhân viên.'; RETURN; END;

    INSERT INTO PHONGBAN(MAPHG,TENPHONG,TRPHG,NG_NHANCHUC)
    VALUES(@MAPB,@TenPB,@TrPHG,@NgNhanChuc);
    PRINT N'Thêm phòng ban thành công.';
END
GO

-- j. Cập nhật ngày kết thúc dự án
CREATE PROCEDURE usp_UpdateProjectEndDate
    @MADA INT,
    @NgayKT DATETIME
AS
BEGIN
    IF NOT EXISTS(SELECT 1 FROM DEAN WHERE MADA = @MADA)
    BEGIN PRINT N'Mã dự án không tồn tại.'; RETURN; END;

    DECLARE @NgayBD DATETIME;
    SELECT @NgayBD = NGAYBD FROM DEAN WHERE MADA = @MADA;
    IF @NgayKT < @NgayBD
    BEGIN PRINT N'Ngày kết thúc phải sau ngày bắt đầu.'; RETURN; END;

    UPDATE DEAN SET NGAYKT = @NgayKT WHERE MADA = @MADA;
    PRINT N'Cập nhật ngày kết thúc thành công.';
END
GO

-- k. Phân công nhân viên vào dự án mới với kiểm tra vị trí
...
BEGIN
    ...
    BEGIN PRINT N'Vị trí không hợp lệ.'; RETURN; END;
    IF @VITRI = N'Trưởng Dự Án' AND EXISTS(
        SELECT 1 FROM PHANCONG WHERE MADA = @MADA AND VITRI = N'Trưởng Dự Án')
    BEGIN PRINT N'Dự án đã có Trưởng Dự Án.'; RETURN; END;
