...
create table cthd(
	...
	constraint fk_sanpham_hoadon foreign key(sohd) references hoadon(sohd)
)
go
set dateformat dmy
go
...
select cthd.sohd,count(distinct sp.masp) from cthd
	inner join sanpham as sp on sp.masp = cthd.masp
where sp.nuocsx = 'Singapore'
group by cthd.sohd
having count(distinct sp.masp) = (select count(*) from sanpham where nuocsx = 'Singapore')
--19 Tìm số hóa đơn trong năm 2006 đã mua ít nhất tất cả các sản phẩm do Singapore sản xuất.
select cthd.sohd, count(distinct sp.masp) as tong from cthd
	inner join sanpham as sp on sp.masp = cthd.masp
	inner join hoadon as hd on hd.sohd = cthd.sohd
where sp.nuocsx = 'Singapore' and year(hd.nghd) = 2006
group by cthd.sohd
having count(sp.masp) = (select count(*) from sanpham
							where nuocsx = 'Singapore')
-- 20 Có bao nhiêu hóa đơn không phải của khách hàng đăng ký thành viên mua?
select [Chưa đk] = count(*)
from hoadon as hd
where hd.makh is null
-- 21. Có bao nhiêu sản phẩm khác nhau được bán ra trong năm 2006.
select count(*) as soluong from
	(select cthd.masp, count(cthd.masp) as soluog from cthd
	join hoadon as hd on hd.sohd = cthd.sohd
	where year(hd.nghd) = 2006
	group by cthd.masp) as dp
-- 22 Cho biết trị giá hóa đơn cao nhất, thấp nhất là bao nhiêu ?
select 
	(select top 1 trigia from hoadon order by trigia desc) as maximum,
	(select top 1 trigia from hoadon order by trigia asc) as minimum
-- 23 Trị giá trung bình của tất cả các hóa đơn được bán ra trong năm 2006 là bao nhiêu?
select AVG(trigia) as trungbinh from hoadon
where year(nghd) = 2006
-- 24 Tính doanh thu bán hàng trong năm 2006.
select sum(trigia) as tong from hoadon
where year(nghd) = 2006
-- 25 Tìm số hóa đơn có trị giá cao nhất trong năm 2006.
select top 1 trigia from hoadon
order by trigia desc
-- 26. Tìm họ tên khách hàng đã mua hóa đơn có trị giá cao nhất trong năm 2006.
select  top 1 kh.hoten from khachhang as kh
...
select  top 3 kh.makh, kh.hoten from khachhang as kh
	order by doanhso desc
-- 28 In ra danh sách các sản phẩm (MASP, TENSP) có giá bán bằng 1 trong 3 mức giá cao nhất
select * from sanpham
where gia in (select distinct top 3 gia from sanpham order by gia desc)
-- 29 In ra danh sách các sản phẩm (MASP, TENSP)
...
select top 3 
    ...
    kh.hoten, 
    kh.doanhso,
    rank() over (order by kh.doanhso desc) as hang
from khachhang as kh
-- 32 Tính tổng số sản phẩm do “Trung Quoc” sản xuất.
select count(*) from sanpham
where nuocsx = 'Trung Quoc'
-- 33 Tính tổng số sản phẩm của từng nước sản xuất.
select nuocsx , count(nuocsx) as tongSanPham from sanpham
group by nuocsx
-- 34 Với từng nước sản xuất, tìm giá bán cao nhất, thấp nhất, trung bình của các sản phẩm.
select sp.nuocsx,max(sp.gia) as lonNhat, min(sp.gia) as nhoNhat, avg(sp.gia) as trungBinh from sanpham as sp
group by sp.nuocsx
-- 35. Tính doanh thu bán hàng mỗi ngày.
select hoadon.nghd,sum(hoadon.trigia) as doanhThu from hoadon
group by hoadon.nghd
-- 36 Tính tổng số lượng của từng sản phẩm bán ra trong tháng 10/2006.
select sp.tensp, sum(cthd.sl) as tongSoLuong from cthd
join hoadon as hd on hd.sohd = cthd.sohd
join sanpham as sp on sp.masp = cthd.masp
where year(hd.nghd) = 2006 and month(hd.nghd) = 10
group by sp.tensp
-- 37 Tính doanh thu bán hàng của từng tháng trong năm 2006.
select month(hd.nghd) as thang, sum(hd.trigia) as doanhthu from hoadon as hd
where year(hd.nghd) = 2006
group by month(hd.nghd)
-- 38 Tìm hóa đơn có mua ít nhất 4 sản phẩm khác nhau.
select hd.sohd,count(distinct cthd.masp) as soLuong from hoadon as hd
inner join cthd on cthd.sohd = hd.sohd
group by hd.sohd
having count(distinct cthd.masp) > 4
-- 39 Tìm hóa đơn có mua 3 sản phẩm do “Viet Nam” sản xuất (3 sản phẩm khác nhau).
select  hd.sohd,count(distinct sp.masp) so_luong from hoadon as hd
inner join cthd on cthd.sohd = hd.sohd
inner join sanpham as sp on sp.masp = cthd.masp
...
select top 1 kh.makh, count(hd.sohd) as SoLanMua from khachhang as kh
inner join hoadon as hd on hd.makh = kh.makh
group by kh.makh
order by SoLanMua desc
-- 41 Tháng mấy trong năm 2006, doanh số bán hàng cao nhất ?
select top 1 month(hd.nghd) as thang, sum(hd.trigia) as doanhthu from hoadon as hd
where year(hd.nghd) = 2006
group by month(hd.nghd)
order by doanhthu desc
-- 42 Tìm sản phẩm (MASP, TENSP) có tổng số lượng bán ra thấp nhất trong năm 2006.
select top 1 cthd.masp,sum(cthd.sl) as tongSanLuong from cthd
inner join sanpham as sp on cthd.masp = sp.masp
inner join hoadon as hd on hd.sohd = cthd.sohd
where year(hd.nghd) = 2006
group by cthd.masp
order by tongSanLuong asc
-- 43 *Mỗi nước sản xuất, tìm sản phẩm (MASP,TENSP) có giá bán cao nhất.
select sp.nuocsx,max(sp.gia) from sanpham as sp
group by sp.nuocsx
-- 44. Tìm nước sản xuất sản xuất ít nhất 3 sản phẩm có giá bán khác nhau.
select sp.nuocsx,count(distinct sp.gia) as sanPham from sanpham as sp
group by sp.nuocsx
having count(distinct sp.gia) > = 3
-- 45 *Trong 10 khách hàng có doanh số cao nhất, tìm khách hàng có số lần mua hàng nhiều nhất.
select top 1 kh.makh, count(hd.sohd) as tong from khachhang as kh
inner join hoadon as hd on hd.makh = kh.makh
group by kh.makh
order by tong desc